build_forest_30sec_grid <- function(job_id, id_hansen, area, year, grid_30sec, sf_list, output_path, log_file = NULL, ncores = 1){
  ...
  block_values <- lapply(1:r_blocks$n, function(b){
    ...
    forest_timeseries <- sub_tile_tbl %>% 
    ...
    sf::st_write(sub_tile_tbl, dsn = fname_grid_sf, factorsAsCharacter = TRUE, delete_dsn = TRUE)
    readr::write_csv(forest_timeseries, path = fname_forest_ts)
